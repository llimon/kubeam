...
BIN      = $(GOPATH)/bin
BASE     = $(GOPATH)/src/github.com/kubeam/$(PACKAGE)
...
.PHONY: all
all: fmt lint vendor cert package | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
...
$(BASE): ; $(info $(M) setting GOPATH…)
	@mkdir -vp $(dir $@)
	#@mkdir -vp $(dir $@)/github.com/kubeam
	#@ln -sf $(CURDIR) $(dir $@)/github.com/kubeam
	@ln -sf $(CURDIR) $@
